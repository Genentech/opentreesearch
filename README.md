...
This repo implements tree search on top of OpenEvolve. Tree Search uses a different, simpler evolutionary algorithm for LLM-guided code evolution. Unlike OpenEvolve which has at least 9 different hyperparameters for island-based evolution, Tree Search only has 1 hyperparameter, and its default value is sufficient for all use cases we have tried.

Open Tree Search is based on the paper "An AI system to help scientists write expert-level empirical software" from Google Research. See https://research.google/blog/accelerating-scientific-discovery-with-ai-powered-empirical-software/ and https://arxiv.org/abs/2509.06503. Our implementation follows the appendix in the paper.

New code for tree search is in `openevolve/tree_search/`.

Env:

```bash
conda create -n treesearch python=3.10
...
# Run toy evolution
# will write output to output/func_min/
python treesearch-run.py examples/function_minimization/initial_program.py \
  examples/function_minimization/evaluator.py \
  --config examples/function_minimization/config-treesearch.yaml \
  --output output/func_min/run_2/
```

The only config setting unique to tree search is:

```yaml
database:
  puct_exploration_constant: 1
```

Example evolved tree (function minimization task):

<img src="treesearch-example.png">
