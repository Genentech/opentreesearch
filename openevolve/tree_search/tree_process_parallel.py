...
from openevolve.tree_search.tree_database import TreeProgramDatabase, TreeNode
from openevolve.process_parallel import ProcessPoolExecutor, SerializableResult, _lazy_init_worker_components
...
class TreeParallelController:
    """ Controller for tree search.
        Built off of openevolve.process_parallel.ProcessParallelController.

        Not implemented here, but could be added by refering to the original controller (may generally require changes in here, TreeNode, and TreeProgramDatabase):
        - Artifacts
        - Novelty scoring by LLM embeddings

    """
    def __init__(self, config: Config, evaluation_file: str, database: TreeProgramDatabase, evolution_tracer=None, file_suffix: str = ".py"):
        self.config = config
        ...
        logger.info(f"Initialized tree search process parallel controller with {self.num_workers} workers")
    ...
    def _create_database_snapshot(self) -> Dict[str, Any]:
        ...
        snapshot = {
            "tree": {pid: node.to_dict() for pid, node in self.database.tree.items()},
        }
        return snapshot
    ...
    async def run_evolution(
        ...
        logger.info(
            f"Starting tree search evolution from iteration {start_iteration} "
            f"for {max_iterations} iterations (total: {total_iterations})"
        )

        pending_futures: Dict[int, Future] = {}

        # Submit initial iterations
        ...
        while (
            ...
            try:
                ...
                if result.error:
                    ...
                    child_program = TreeNode(**result.child_program_dict)
    ...
    def _submit_iteration(self, iteration: int) -> Optional[Future]:
        """ Submit an iteration to the process pool.
            ...
            Currently does not benefit from parallelism, because given a tree, ucb_expand is deterministic.
            TODO - update ucb_expand to return a list of parents to expand, and submit multiple iterations in parallel.
        """
        try:
            parent, inspirations = self.database.ucb_expand(num_inspirations=self.config.prompt.num_top_programs)

            # Create database snapshot
            db_snapshot = self._create_database_snapshot()
            db_snapshot["sampling_node_id"] = parent.id
