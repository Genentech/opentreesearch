"""
Main controller for TreeSearch - TODO
"""
...
from openevolve.tree_search.tree_database import TreeNode, TreeProgramDatabase
from openevolve.tree_search.tree_process_parallel import TreeParallelController
...
class TreeSearch:
    """
    Main controller for OpenEvolve with TreeSearch
    ...
    def __init__(
        ...
        self.database = TreeProgramDatabase(self.config.database)
    ...
    async def run(
        ...
        should_add_initial = (
            start_iteration == 0
            and len(self.database.tree) == 0
            and not any(
                p.code == self.initial_program_code for p in self.database.tree.values()
            )
        )
    ...
    def _save_best_program(self, program: Optional[TreeNode] = None) -> None:
        """
